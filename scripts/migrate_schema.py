...
def migrate_schema():
    ...
    console.print("\n[bold]Package Migrating history.csv to Simplified Schema[/bold]\n")
    ...
    if not extra_columns:
        console.print("[green]OK Schema already simplified - no migration needed[/green]\n")
        return
    ...
    if 'price' not in df.columns:
        if 'price_eur' in df.columns:
            ...
            console.print("[dim]OK Renamed price_eur to price[/dim]")
        elif 'price_usd' in df.columns:
            df['price'] = df['price_usd']
            console.print("[dim]OK Renamed price_usd to price[/dim]")
        else:
            df['price'] = 0.0
            console.print("[yellow]Warning: No price column found - setting to 0.0[/yellow]")
    ...
    try:
        ...
        console.print("[green]OK Data types validated[/green]")

    except Exception as e:
        console.print(f"[red]Failed Error validating data types: {e}[/red]")
