...
def migrate_csv_to_postgres():
    ...
    print("Run Starting migration from CSV to PostgreSQL...")
    ...
    try:
        ...
        print(f"Failed Failed to initialize database: {e}")
        return

    # Check current wine count
    try:
        current_count = get_wine_count()
        print(f"Results: Current wines in database: {current_count}")
    except Exception as e:
        print(f"Warning:  Could not check current wine count: {e}")
        current_count = 0

    # Load CSV
    print(f"Upload Loading CSV from {csv_path}...")
    try:
        df = pd.read_csv(csv_path)
        print(f"OK Loaded {len(df)} wines from CSV")
    except Exception as e:
    ...
    for idx, row in df.iterrows():
        try:
            ...
            add_wine(wine_data)
            success_count += 1
            print(f"   OK {wine_data['wine_name']}")

        except Exception as e:
            error_count += 1
            print(f"   Failed {row.get('wine_name', 'Unknown')}: {e}")
    ...
    try:
        ...
        print(f"   Results: Total wines in database: {final_count}")
    except Exception as e:
