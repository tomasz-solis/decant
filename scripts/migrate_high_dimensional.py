...
def migrate_to_high_dimensional():
    ...
    if not missing_columns:
        console.print("[green]OK Already HIGH-DIMENSIONAL - no migration needed[/green]\n")
        return
    ...
    if 'wine_color' not in df.columns:
        ...
        df['wine_color'] = 'White'
        console.print("[dim]OK Added wine_color (default: White)[/dim]")

    if 'is_sparkling' not in df.columns:
        df['is_sparkling'] = False
        console.print("[dim]OK Added is_sparkling (default: False)[/dim]")

    if 'is_natural' not in df.columns:
        df['is_natural'] = False
        console.print("[dim]OK Added is_natural (default: False)[/dim]")

    if 'sweetness' not in df.columns:
        df['sweetness'] = 'Dry'
        console.print("[dim]OK Added sweetness (default: Dry)[/dim]")
